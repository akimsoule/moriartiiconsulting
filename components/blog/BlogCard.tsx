...
export default function BlogCard({ post, featured = false }: BlogCardProps) {
  return (
    ...
    >
      <div className={`${featured ? 'h-64 lg:h-full' : 'h-48'} relative bg-gray-200`}>
        <img 
      ...
      <div className="p-6">
        ...
        <h3 className={`font-serif font-bold text-moriartii-primary ${
          featured ? 'text-2xl mb-3' : 'text-xl mb-2'
        }`}>
          <Link href={`/blog/${post.slug}`} className="hover:text-moriartii-secondary transition-colors duration-300">
            {post.title}
          </Link>
        </h3>
